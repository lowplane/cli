...
This is the **open-source** Sevro CLI (`@sevro/cli`). It must remain independently auditable as Apache-2.0 OSS — that is the entire reason it lives in a separate repo from the proprietary backend. Strategy reference: [../docs/open_source_cli_playbook.md](../docs/open_source_cli_playbook.md).
...
- `demo` — runs analysis on a bundled demo chart so people can `npx @sevro/cli demo` and see output without supplying input.
